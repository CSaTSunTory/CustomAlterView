...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        ...
        label.text = message;
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, 80)];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if (buttonTitles.count ==2) {
            UIImageView *view = [[UIImageView alloc]initWithFrame:CGRectMake(  buttonWidth, container.bounds.size.height - buttonHeight, 0.5, buttonHeight)];
            view.backgroundColor =[UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
              [container addSubview:view];
        }
        [closeButton addTarget:self action:@selector(customIOS7dialogButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
        [closeButton setTag:i];
        [closeButton setTitle:[buttonTitles objectAtIndex:i] forState:UIControlStateNormal];
        [closeButton setTitleColor:_titleColor forState:UIControlStateNormal];
...
+(void)showMessage:(NSString *)mess{
    CustomIOSAlertView *cust = [[CustomIOSAlertView alloc] init];
    cust.message = mess;
    [cust show];
